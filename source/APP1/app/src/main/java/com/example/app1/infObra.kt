...
class infObra : DialogFragment() {
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        ...
        var v:View?=activity?.layoutInflater?.inflate(R.layout.fragment_inf_obra, null, false)
        Log.i("prueba","uno")
        inicializar(v)
        accionar()
        b.setView(v)
    ...
    fun inicializar(view:View?){
        try {
            Log.i("prueba","4")
            this.exitFile=view?.findViewById(R.id.exitObra)
            this.notifyPath=view?.findViewById(R.id.messagePathObra)
            this.selectFile=view?.findViewById(R.id.selectInfObra)
            this.nameFile=view?.findViewById(R.id.nameFileObra)
        ...
        try {
            ...
            this.selectFile?.setOnClickListener{view ->
                ...
                if(this.nameFile?.text.toString().trim().length>0){
                    ...
                    this.notifyPath?.text="Por favor digite el nombre del informe Obra"
                }
            }
            ...
            try {
                ...
                storeRef.putFile(saveUri!!).addOnSuccessListener {
                    ...
                    storeRef.downloadUrl.addOnSuccessListener { uri ->
                        notifyPath?.text = "ARCHIVO DE OBRA CARGADO CON EXITO!!"
                        Log.i("path", uri.toString())
                        var db = FirebaseFirestore.getInstance()
                        val dbDocument = db.collection("obraDocuments")
